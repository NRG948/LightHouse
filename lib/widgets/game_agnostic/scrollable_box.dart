...
class ScrollableBox extends StatefulWidget {
  ...
  final List<List<String>> comments; // List of comments [Scouter Name, Text, Match]
  final Sort sort; // Sorting option
...
class _ScrollableBoxState extends State<ScrollableBox> {
  ...
  Sort get _sort => widget.sort; // Getter for sorting option
  ...
  void setSortingAlgorithm() {
    switch (_sort) {
      case Sort.LENGTH_MAX:
        sortingAlgorithm = (x, y) => y[1].length - x[1].length; // Sort by comment length (descending)
      case Sort.NAME_AZ:
        sortingAlgorithm = (x, y) => x[0].compareTo(y[0]); // Sort by scouter name (A-Z)
      case Sort.NAME_ZA:
        sortingAlgorithm = (x, y) => y[0].compareTo(x[0]); // Sort by scouter name (Z-A)
      case Sort.EARLIEST:
        sortingAlgorithm = (x, y) => int.parse(x[2]) - int.parse(y[2]); // Sort by match number (ascending)
      case Sort.LATEST:
        sortingAlgorithm = (x, y) => int.parse(y[2]) - int.parse(x[2]); // Sort by match number (descending)
    }
  }
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(Constants.borderRadius)), // Set the border radius
        child: Column(
            ...
            Expanded(
              child: ListView.separated(
                  ...
                  separatorBuilder: (context, index) =>
                      Divider(height: 12, color: Constants.pastelWhite), // Divider between comments
